...
def message_body_update(message):
    message_body = "log entry at {}: {} \n".format(
        static_references.time_stamp, message
    )
...
def zip_files():
    ...
    try:
        ...
        message_body_update(
            "LargeZipFile exception raised as {} {}".format(e, func_name)
        )
...
def check_for_content_to_mail(email):
    ...
    try:
        ...
        zip_list = [
            attachment for attachment in mail_list if attachment.endswith(".zip")
        ]
...
def create_error_log(message):
    ...
    file_name = os.path.abspath(
        "{}error_log{}.csv".format(static_references.log_directory, str(day_stamp))
    )
...
def mailer(zipped_list, email=static_references.default_email):
    ...
    try:
        ...
        create_error_log(
            "error: The server unexpectedly disconnects, {}".format(func_name)
        )
        ...
        create_error_log(
            "error: The SMTP server refused to accept the message data, {}".format(
                func_name
            )
        )
        ...
        create_error_log(
            "error: Error occurred during establishment of a connection with the server, {}".format(
                func_name
            )
        )
        ...
        create_error_log(
            'error: The server refused our "HELO" message, {}'.format(func_name)
        )
